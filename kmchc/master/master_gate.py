...
_NUMERIC_CHOICE = re.compile(
    r'^\s*(?:[가-힣a-zA-Z]{1,4}\s+)?'             # ★머리 낱말(오비탈·전자·약 …) — T13 P16
    r'[−\-]?\d[\d,]*(?:\.\d+)?\s*'              # 수치 본체
    r'(?:×\s*10\s*[-−]?\d+)?\s*'                # 지수 표기(예외적으로 허용)
    r'(?:[%가-힣a-zA-Z°Ω/·\s]{0,4})?\s*$')      # 짧은 단위 꼬리(분·년·배·번·g·% …)
#   ★T13 P16 이 부른 확장★ — 종전에는 보기가 ★숫자로 시작할 때만★ 수치 보기로 보았다.
#   그래서 단위 낱말을 앞에 붙이면('오비탈 9 개' · '전자 20 개') G6 을 비켜 갔고, 한
#   배치 안에서 어떤 수치 보기는 오름차순이고 어떤 것은 아닌 상태가 됐다. 머리 낱말을
#   한 마디까지 허용해 ★수가 들어 있는 짧은 보기★ 를 모두 검사한다.
#   ▸ 은행 2,130 제에 돌려 새로 걸린 것은 P16 의 두 자리뿐이었고 둘 다 바로잡았다.
