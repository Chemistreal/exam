...
MK = ['①', '②', '③', '④']
_POSNOTE = re.compile(r'(옳음|옳아|참임|이 참|맞음|답임|이 답|정답)')
...
def core(items, rep):
    ...
    for it in items:
        ...
        #   ▸ ★'[정답] ③ …' 도 확정 어구다★ — T1~T13 의 해설은 이 꼴로 정답을 못 박는다
        #     (T.sol 이 지금도 이 머리글을 붙인다). 이 꼴을 안 보아서 ★1,745건이 유령 부채★ 로
        #     쌓여 있었다 — 전건이 '[정답] N' 을 지니고 있었다. 검사의 구멍이 부채로 보인 자리다.
        if not (any(t in sol for t in ('가 옳아', '이 옳아', f'답이 {MK[a]}', f'[정답] {MK[a]}'))
        ...
        mid = (L[1] + L[2]) / 2
        #  ★선지가 넷 다 16자 미만이면 산포 규약이 서지 않는다★ — 오늘의 규약(평서문 16~24자)이
        #  서기 전에 지은 문항은 '4.0 atm' · '3' 처럼 수치·짧은 말을 선지로 쓴다. 그 자리에서
        #  (최장−최단)÷평균은 뜻이 없다(128건이 그런 자리였다). 오늘 규약을 지키는 문항은
        #  넷 다 16자 이상이므로 이 울타리가 새 흠을 가리지 않는다.
        if L[3] < 16:
            pass
        elif mid >= 8 and (L[3] - L[0]) / mid > 0.25:
        ...
        # ── X ★계산 줄이 정답을 오답처럼 적은 자리★ (T22 감사41 이 세웠다) ──
        #   calc 의 꼬리는 '① … / ② … / ④ …' 처럼 ★오답 셋★ 을 번호로 짚는 묶음이다.
        #   그 셋 가운데 정답 번호가 있으면 ★번호가 밀린★ 것이다 — 학생이 정답을 오답으로 읽는다.
        #   ▸ ★낱말(아니·틀·반대)로 세면 놓친다★ — M02634 의 '③ 탄소 수가 같아도 꼴이 다르면
        #     다른 물질' 에는 그런 낱말이 없다. 신호는 낱말이 아니라 ★자리★ 다.
        #   ▸ ★단평이 넷이면 짚지 않는다★ — 네 선지를 모두 늘어놓는 셈 줄(양자수 테마)에서는
        #     정답 번호를 다는 것이 규약에 맞다. 셋일 때만 '오답 셋' 묶음으로 읽는다.
        _segs = [x.strip() for x in (it.get('calc_check') or '').split('/')]
        _notes = [(MK.index(x[0]), x) for x in _segs if x and x[0] in MK]
        if len(_notes) == 3:
            for _n, _seg in _notes:
                if _n == a and not _POSNOTE.search(_seg):
                    rep(it, 'X', '단평 셋 가운데 정답 %s 를 달고 있다 — %s' % (MK[a], _seg[:44]))
                    break
