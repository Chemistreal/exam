...
var MSG_QUOTES = {
  high: ['“재능은 노력을 이기지 못하고, 노력은 즐기는 자를 이기지 못한다.”', '“정상에 오르는 길은 늘 오르막이다 — 지금의 실력이 그 증거입니다.”'],
  mid: ['“천 리 길도 한 걸음부터. 오늘 짚은 한 개념이 내일의 실력이 됩니다.”', '“꾸준함은 재능을 이깁니다. 지금의 방향이 맞습니다.”'],
  low: ['“틀린 문제는 가장 좋은 스승입니다. 오늘의 오답이 내일의 정답이 됩니다.”', '“실패는 성공의 어머니 — 지금 메우는 개념 하나가 판을 바꿉니다.”']
};
...
function _buildReportMsg(title, name, correct, pct100, perc, areasStr, link, qi) {
  ...
  var band = pct100 >= 80 ? 'high' : (pct100 >= 55 ? 'mid' : 'low');
  var analysis;
  if (cfg.mode === 'tier') {
    var aw = _award(wrong);
  ...
  var strongTxt = strong.length ? '특히 ' + strong.join(', ') + ' 영역에서 안정적인 강점을 보였습니다.' : '전 영역에서 고르게 응답했습니다.';
  var weakTxt = weak.length ? '반면 ' + weak.join(', ') + ' 영역은 보완이 필요합니다 — 아래 성적표의 개념 클리닉으로 우선 복습을 권합니다.'
    : '뚜렷한 취약 영역 없이 균형이 좋습니다. 심화 문항으로 난도를 올려 보세요.';
  var closing = band === 'high' ? '지금의 실력을 유지하며 고난도 문항에서의 회복력을 키우면 최상위권이 충분히 가능합니다.'
    : band === 'mid' ? '방향이 정확합니다. 오답 개념만 촘촘히 메우면 다음 시험에서 확실한 도약이 기대됩니다.'
    : '기초 개념부터 하나씩 다시 세우면 반드시 오릅니다. 끝까지 함께 책임지고 끌어올리겠습니다.';
  var quote = MSG_QUOTES[band][qi % MSG_QUOTES[band].length];
  return ['안녕하세요. 화학올림피아드 담당하는 조준모입니다.',
    '오늘 ' + nm + ' 학생이 응시한 「' + title + '」' + _EN(title) + ' ' + cfg.topic + _EL(cfg.topic) + ' 다룬 시험이었습니다.',
    nm + ' 학생은 ' + analysis,
    strongTxt + ' ' + weakTxt,
    quote, closing,
    '아래 링크에서 문항별 정오와 개념 클리닉, 학습 처방을 확인하실 수 있습니다.',
    link, '— 다원교육 영재관 · 화학 조준모 올림'].join('\n');
}
