/* ============================================================
   ...
   ── 이 검사가 **막지 못하는 것** (2026-09-07 에 적어 둔다) ──────────────
   이 검사는 **눈에 보이는 링크**를 막는다. 정답을 숨기지는 못한다.

     · `exams.json` 의 `key` 배열이 `final.html` 안에 `FALLBACK_EXAMS` 로
       통째로 박혀 있다 — 소스 보기에서 `key` 를 찾으면 나온다
     · `answers/*.json` 도 공개 경로에서 그냥 받아진다

   채점이 브라우저에서 일어나는 한 정답은 브라우저에 있어야 한다. 구조라서
   못 막는다. 그러니 **이 검사가 「정답이 안 새어 나간다」를 지키고 있다고
   읽으면 안 된다.** 지키는 것은 「답을 넣기 전에 실수로 눌러 보게 되지
   않는다」까지다.

   시험을 정말로 막으려면 채점을 서버로 옮겨야 하고, 그건 다른 공사다.
   지금 동선(시험은 종이·현장에서 보고 화면은 채점·복습에 쓴다)에서는
   이 정도가 맞다 — 다만 그것이 **선택**이라는 것을 다음 사람이 알아야
   해서 여기 적는다.
