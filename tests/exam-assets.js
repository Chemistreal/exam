...
console.log('\n── 해설지가 데이터를 그대로 담는다 ──');
{
  ...
  /* 「확인 필요」는 학생·학부모가 보는 해설지에도 찍힌다.
     ⚠ 예전에는 여기서 «이 회차에는 하나도 없다» 를 못박고 있었다. 그것은
       그때 이 회차가 마침 깨끗했다는 **한 순간의 사진**이지 규칙이 아니었다.
       지금은 스물일곱 회차가 확인 필요를 싣고 있고(생성기가 일부러 그린다),
       이 회차도 진짜 결함 둘을 안고 있다 — 1번은 답이 갈릴 수 있고 16번은
       시험지에 없는 화학종이 인쇄돼 있다. 그걸 지우는 것이 옳을 리 없다.

     그래서 «없어야 한다» 를 «학생에게 보내면 안 되는 말이 섞이면 안 된다» 로
     바꾼다. 안쪽 문서 이름·파일 경로·담당자 메모는 해설지에 나가면 안 된다. */
  ...
  chk('확인 필요가 데이터와 해설지에서 같다',
      /확인 필요/.test(page),
      Object.keys(q).some(k => String(q[k].reviewNote || '').trim()));
