...

{
  ...
  Object.keys(BASE).forEach(id => {
    ...
    /* from:'rate' — 공식 정답률과 응시 인원으로 세운 반쪽 기록이다.
  ...
  /* 1214 → 1227 : 시트에서 KMChC 2025 제1차 심화(7명)·일반(6명)이 들어왔다.
     ...
     1235 → 2758 : 단원별 여덟 회차(화학1·화학2)를 들이면서 1523명이 들어왔다.
     그 여덟은 공식 정답률과 응시 인원만 있는 반쪽 기록(from:'rate')이라
     또래 정답률은 서지만 석차 모집단은 아직 없다 (2026-08-23).
     2758 → 2763 : 기준 기록 자동 갱신(시트)이 5명을 들여왔다 (2026-08-23). */
  chk('기준 기록 인원', Object.keys(BASE).reduce((a, k) => a + BASE[k].n, 0), 2763);
}
...
console.log('\n── 답이 덮인 문항은 지어내지 않는다 ──');
{
  ...
  /* 분포가 통째로 없는 문항은 분포를 **지어내지 않는다**(null) — 두어 명의
     선택으로 막대를 그리면 「누적 N명」 이라는 머리말과 다른 말이 된다. */
  chk('분포 없는 문항은 분포를 지어내지 않는다', cs.qopt[46], null);
