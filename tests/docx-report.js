...
(async()=>{
  ...
  /* '다음 진단 추천'은 선생님이 다음 회차를 고를 때 쓰는 안내다. 학부모가 받는
     종이에는 할 일이 아니라 뺐다. 화면에는 그대로 있어야 한다 — 섹션을 통째로
     지운 게 아니라 그 줄만 빠지는 것이므로, 숙달 현황 자체는 남아야 한다. */
  chk('다음 진단 추천은 Word 에 없다',/다음 진단 추천/.test(txt),false);
  chk('숙달 현황 본문은 그대로 있다',/숙달 추적|재출제/.test(txt),true);
