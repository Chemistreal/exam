...
WAITING = dict(
    ...
    **{i: '⚖ A8 — 2017~2026 인데 엑셀을 아직 못 받았다'
    ...
    # 조준모의고사 0회 — 선생님이 적어 주신 응시 인원은 **57명**인데(2026-08-31 목록),
    # 지금 분모는 시트에 있는 18명뿐이다. j0_data.json 에 40명분 총점이 있으나
    # 그중 열하나가 3의 배수가 아니라(문항당 3점으로 채점된 것이 아니라는 뜻)
    # 3으로 나눠 맞은 문항 수를 만들면 없는 등수를 지어내게 된다.
    # 57명분 원자료를 받아야 채워진다.
    **{'j0': '선생님이 적으신 응시 57명 — 지금 분모는 시트의 18명. 원자료를 받아야 한다'})
