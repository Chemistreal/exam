...
2026-08-11 · crops 를 줄일 수 있나 (선생님 결정 #47)
----------------------------------------------------
재어 보고 **안 줄이기로 한다.** 2,310장 51.7MB 인데 —

  · 전부 이미 **회색조(L)** 다. 색을 뺄 것이 없다
  · 무손실로 다시 저장하면 **0% 줄어든다** — 이미 바닥이다
  · 64색 팔레트로 낮추면 22%(약 11MB) 준다. 그런데 그건 **문항 그림을 깎는
    것**이다. 아래첨자·전하·그래프 눈금이 학생이 실제로 읽는 자리다

배포 한도는 1GB 이고 지금 181.5MB(18%) 다. 아직 급하지 않은 자리를 위해
학생이 보는 그림을 깎지 않는다. 문턱(700MB)에 가까워지면 그때 다시 본다 —
그 알림이 이 자의 일이다.
"""
import os
import subprocess
