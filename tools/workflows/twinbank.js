...
const results = await pipeline(
  ...
  function (it) {
    ...
    const j0hint = key.indexOf('j0__') === 0
      ? '이 회차는 **문제지 PDF 만** 없다. 원문 지문·선지는 위 파일에 그대로 들어 있으니\n'
        + '다른 회차와 똑같이 읽고 쓰면 된다. (2026-09-05 정정: 여기 「원문 지문이 없다」고\n'
        + '적혀 있었는데 틀린 말이었다 — answers/j0.json 에 예순 문항이 다 있다.)\n'
        + '더 볼 것이 있으면 `' + SRC + '/j0q/blk-*.txt` 에 원문 시험의 상세 해설이 문항별로\n'
        + '잘려 있다. 「문제 N ·」 대목에 판단 과정·선지별 분석·핵심 오개념이 있다.\n\n'
      : ''
    const p = '동형문제를 집필한다.\n\n'
