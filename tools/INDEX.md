...
| | 자 | `--check` | 언제 돌리나 | 무엇을 |
|---|---|---|---|---|
| ✓ | `answer_fill.py` | ○ | 밀 때마다 · 하루 한 번 | 집필해 둔 조각을 답지(answers/<id>.json)에 채워 넣는다. |
| ✓ | `answer_sync.py` | ○ | 밀 때마다 · 하루 한 번 | 채점하는 정답과 **해설이 말하는 정답**이 같은지 본다. |
| ✓ | `area_tag.py` | ○ | 밀 때마다 · 하루 한 번 | 회차의 **영역 이름**이 성적표가 아는 이름인지 본다. |
| ✓ | `audit_pages.py` | ○ | 밀 때마다 · 하루 한 번 | 화면을 한 줄씩 재는 자. |
| ✓ | `blind_wait.py` | ○ | 밀 때마다 · 하루 한 번 | 검사가 "이쯤이면 됐겠지" 하고 재우는 시간을 잰다. |
|  | `build_wrongbook_assets.py` |  | 사람이 손으로 | Build and audit wrong-answer review assets for final.html. |
| ✓ | `ci_deps.py` | ○ | 문서를 고칠 때 · 밀 때마다 · 하루 한 번 | CI 가 부르는 자가 **거기서 실제로 돌 수 있는지** 본다. |
| ✓ | `cohort_cover.py` | ○ | 밀 때마다 · 하루 한 번 | **모든 시험에 기준 기록(연도누적 인원)이 들어 있는지** 본다. |
...
| ✓ | `cut_link.py` | ○ | 밀 때마다 · 하루 한 번 | 늦춘 기준이 **시험지**를 보고 늦춘 것인지, 그 회차 **사람들**을 보고 늦춘 것인지. |
| ✓ | `dh_absorb.py` | ○ | 밀 때마다 · 하루 한 번 | 집필한 동형문제 조각을 은행으로 옮기면서 **개념 이름을 답지와 맞춘다.** |
|  | `dh_concept_scan.py` |  | 사람이 손으로 | 원 DB의 `concept` 라벨에서 오타 후보를 찾는다. |
| ✓ | `dh_dupe_scan.py` |  | 밀 때마다 · 하루 한 번 | 서로 같은 문항이 된 쌍을 찾는다. 한 시험 안에서도, 시험끼리도 본다. |
| ✓ | `dh_lint.py` |  | 밀 때마다 · 하루 한 번 | 동형문제 상시 검사. 사람이 볼 수 없는 규모(2400문항)를 기계가 매번 훑는다. |
|  | `dh_manifest.py` |  | 사람이 손으로 | 동형문제 재집필용 입력 매니페스트 생성. |
|  | `dh_merge.py` |  | 사람이 손으로 | 집필된 파트 JSON들을 합쳐 donghyung/<examId>.json 을 만든다. |
|  | `dh_normalize.py` |  | 사람이 손으로 | 재집필본의 ASCII 화학 표기를 유니코드로 바꾼다(원칙 9). |
| ✓ | `dh_number.py` | ○ | 밀 때마다 · 하루 한 번 | 기출동형 문항의 **셈이 고른 답과 맞는지** 본다. |
|  | `dh_rebalance.py` |  | 사람이 손으로 | 한 번호에 몰린 정답을 고르게 흩는다. |
|  | `dh_review.py` |  | 사람이 손으로 | 집필된 파트 JSON을 검수용 압축 형식으로 출력. |
| ✓ | `dh_validate.py` |  | 밀 때마다 · 하루 한 번 | 집필된 동형문제(schemaVersion 2, strategy=original-authored) 구조 검증. |
| ✓ | `doc_links.py` | ○ | 문서를 고칠 때 | 문서에 적은 **다른 문서 이름이 눌러서 열리는지** 본다. |
| ✓ | `dupe_pages.py` | ○ | 밀 때마다 · 하루 한 번 | 같은 화면이 두 이름으로 있는 것을 찾고, **한쪽만 고치는 것**을 막는다. |
| ✓ | `editor_note.py` | ○ | 밀 때마다 · 하루 한 번 | 선생님께 남긴 메모가 학생 화면으로 새어 나가지 않는지 본다. |
| ✓ | `font_block.py` | ○ | 밀 때마다 · 하루 한 번 | 바깥 글꼴이 **첫 화면을 인질로 잡는 것**을 푼다. |
| ✓ | `haeseol_index.py` | ○ | 밀 때마다 · 하루 한 번 | 해설 목차(`index_haeseol.html`)가 실제 회차와 맞는지 본다. |
| ✓ | `hub_audit.py` | ○ | 밀 때마다 · 하루 한 번 | 허브(`hub.html`)의 **0회차 표**를 다시 잰다 — 브라우저 없이. |
|  | `hwp_text.py` |  | 사람이 손으로 | HWP 5.0(바이너리) 문제지에서 글자를 뽑는다 — **수식까지 제자리에**. |
|  | `hwpx_exam.py` |  | 사람이 손으로 | 선생님이 한글(HWPX)로 만든 시험지를 읽어 구조로 바꾼다. |
|  | `hwpx_render.py` |  | 사람이 손으로 | HWPX 에서 캔 문제 덩이를 시험지 HTML 로 옮긴다. |
...
| ✓ | `lec_readtime.py` | ○ | 밀 때마다 · 하루 한 번 | 강의마다 **읽는 데 얼마나 걸리는지** 적는다. |
| ✓ | `lec_selfcheck.py` | ○ | 밀 때마다 · 하루 한 번 | 숙제에 **스스로 확인할 기준**이 붙어 있는지 본다. |
| ✓ | `lecture_link.py` | ○ | 밀 때마다 · 하루 한 번 | 틀린 문항이 **정확히 어느 개념강의 한 강**으로 가는지 지킨다. |
|  | `lecture_map.py` |  | 사람이 손으로 | **어느 강의가 어느 개념인가** — 125강의 개념 표. |
| ✓ | `lecture_quiz.py` | ○ | 밀 때마다 · 하루 한 번 | 강의 125강에 **확인 문제**를 붙인다 — 이미 사람이 검수한 문항에서 골라서. |
| ✓ | `lecture_sync.py` | ○ | 밀 때마다 · 하루 한 번 | 틀린 문항이 **실제로 있는 강의**로 가는지 지킨다. |
|  | `legacy_figs.py` |  | 사람이 손으로 | 단원별 여덟 회차에서 그림이 소실된 문항의 그림을 다시 그린다. |
| ✓ | `lie_check.py` | ○ | 밀 때마다 · 하루 한 번 | **자가 거짓말을 하는지** 잰다 — 참·거짓 예시를 주고 맞히는지 본다. |
| ✓ | `mis_sync.py` | ○ | 밀 때마다 · 하루 한 번 | **오개념 카탈로그와 강의의 함정이 같은 말을 하는지** 본다. |
| ✓ | `msg_ledger.py` | ○ | 밀 때마다 · 하루 한 번 | 화면이 **사람에게 하는 말**을 한 대장에 모은다. |
| ✓ | `name_guard.py` |  | 밀 때마다 · 하루 한 번 | 공개 저장소에 학생 실명이 다시 들어오는 것을 막는다. |
...
| ✓ | `report_data.py` | ○ | 밀 때마다 · 하루 한 번 | 회차마다 **성적표가 무엇을 말할 수 있고 무엇이 비는지** 한 판에 편다. |
...
| ✓ | `score_rule.py` | ○ | 밀 때마다 · 하루 한 번 | **시험지에 인쇄된 배점 규칙**과 채점기가 쓰는 배점을 맞대어 본다. |
| ✓ | `seed_stats.py` | ○ | 밀 때마다 · 하루 한 번 | index.html 의 두 코호트가 서로 어긋나지 않는지 본다 — SEEDS 와 STATIC_STATS. |
| ✓ | `start_index.py` | ○ | 밀 때마다 · 하루 한 번 | `START_HERE_index.html` 이 실제 파일과 맞는지 본다. |
| ✓ | `store_ledger.py` | ○ | 밀 때마다 · 하루 한 번 | 이 브라우저에 **무엇을 남기는지** 적어 두고, 늘어나면 한 번 묻게 한다. |
| ✓ | `submit_link.py` | ○ | 밀 때마다 · 하루 한 번 | 학생이 **스스로 풀고 내는 문**이 회차마다 제대로 서 있는지 본다. |
| ✓ | `sw_freshness.py` | ○ | 밀 때마다 · 하루 한 번 | **낡아도 되는 파일과 낡으면 안 되는 파일**을 서비스워커가 갈라 놓았는지 본다. |
...
| | 자 | `--check` | 언제 돌리나 | 무엇을 |
|---|---|---|---|---|
|  | `gen_chunks.py` |  | 만들 때 사람이 | 집필 에이전트에게 건네줄 **재료 묶음**을 저장소에서 다시 만들어 낸다. |
...
자 109개 · 판에 걸린 것 90개 · `--check` 를 받는 것 76개 · 아무도 안 돌리는 것 0개 · 표에는 있는데 없는 자물쇠 0개
